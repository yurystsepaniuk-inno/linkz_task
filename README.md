...
```bash
# reservation-api — 15 tests, 5 suites (Jest)
...
- **Stale-payment webhook isolation:** the webhook handler runs in a single transaction. It first updates the `reservations` row anchored to `AND session_id = $sessionId AND status = 'PENDING_PAYMENT'` with `RETURNING seat_id`, and only updates the `seats` row if that reservation UPDATE actually matched. The seat is updated using the `seat_id` from the matched reservation — never from the (untrusted) webhook payload. A late payment event from a previous checkout session matches no live reservation, so the seat — which may now belong to a different user — is left completely untouched. Without this gating, a stale event could flip a seat that a different user is now actively paying for.
...
| **Stale webhook from User A arrives while seat is `PENDING_PAYMENT` for User B** | The `AND session_id = $sessionId` clause matches no live reservation row. Because the seat UPDATE is gated on that match, **both** User B's reservation row **and** the seat row are untouched. Response is still `{received:true}`. | Each webhook is anchored to the unique checkout session ID stored on the `reservations` row. The seat is only ever updated via the `seat_id` returned by a matched reservation, so a stale event cannot flip a seat that another user now holds. |
| **Late webhook for a seat that timed out and is now `CONFIRMED` by someone else** | The reservations UPDATE matches no row (the original reservation is `EXPIRED`), so the seat UPDATE is skipped entirely; the second user keeps the seat. The first user has paid at payment-api with no corresponding reservation. **This is the known reconciliation gap.** See below. | |
